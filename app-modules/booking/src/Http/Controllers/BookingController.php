...
class BookingController extends Controller
{
    ...
    public function indexJson(Request $request)
    {
        ...
        return DataTables::eloquent($model)
            ...
            ->addColumn('actions', function (Booking $booking) {
                ...
                $actions .= '<a href="' . route('booking::admin.bookings.show', $booking->id) . '" class="inline-flex items-center px-2 py-1 text-xs font-medium text-blue-700 bg-blue-100 rounded hover:bg-blue-200" title="View">';
                $actions .= '<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>';
                $actions .= '</a>';
                $actions .= '<a href="' . route('booking::admin.bookings.edit', $booking->id) . '" class="inline-flex items-center px-2 py-1 text-xs font-medium text-yellow-700 bg-yellow-100 rounded hover:bg-yellow-200" title="Edit">';
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('booking::admin.bookings.show', $booking)->with('success', 'Booking created successfully.');
    }
    ...
    public function update(Request $request, Booking $booking)
    {
        ...
        return redirect()->route('booking::admin.bookings.index')->with('success', 'Booking updated successfully.');
    }
}
