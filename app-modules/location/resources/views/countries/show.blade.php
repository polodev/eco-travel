        ...
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-3">
                    <a href="{{ route('admin-dashboard.location.countries.edit', $country->id) }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-yellow-600 border border-transparent rounded-md hover:bg-yellow-700">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                        </svg>
                        Edit
                    </a>
                    <a href="{{ route('admin-dashboard.location.countries.index') }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-600">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <a href="{{ route('admin-dashboard.location.cities.index') }}?country_id={{ $country->id }}" class="text-xs text-blue-600 hover:text-blue-800">View All</a>
                                </div>
                                    ...
                                    <div class="space-y-2">
                                        @foreach($country->cities->take(5) as $city)
                                            <div class="flex items-center justify-between py-1">
                                                ...
                                                <span class="text-xs text-gray-500">{{ $city->airports_count }} airports</span>
                                            </div>
                                        @endforeach
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-3">
                                    <h4 class="font-medium text-gray-800 dark:text-gray-100">Airports ({{ $country->airports->count() }})</h4>
                                    <a href="{{ route('admin-dashboard.location.airports.index') }}?country_id={{ $country->id }}" class="text-xs text-blue-600 hover:text-blue-800">View All</a>
                                </div>
                                @if($country->airports->count() > 0)
                                    <div class="space-y-2">
                                        @foreach($country->airports->take(5) as $airport)
                                            <div class="flex items-center justify-between py-1">
                                                <span class="text-sm text-gray-900 dark:text-gray-100">{{ $airport->name }}</span>
                                                <code class="text-xs bg-gray-200 dark:bg-gray-600 px-1 rounded">{{ $airport->code }}</code>
                                            </div>
                                        @endforeach
                                        @if($country->airports->count() > 5)
                                            <p class="text-xs text-gray-500 pt-1">and {{ $country->airports->count() - 5 }} more...</p>
                                        @endif
                                    </div>
                                @else
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
                        ...
                        <div class="space-y-3">
                            <a href="{{ route('admin-dashboard.location.cities.create') }}?country_id={{ $country->id }}" class="block w-full text-center px-3 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md hover:bg-blue-700">
                                Add City
                            </a>
                            <a href="{{ route('admin-dashboard.location.airports.create') }}?country_id={{ $country->id }}" class="block w-full text-center px-3 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md hover:bg-green-700">
                                Add Airport
                            </a>
                        </div>
                    </div>
