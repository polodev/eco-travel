        ...
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-3">
                    <a href="{{ route('admin-dashboard.location.cities.edit', $city->id) }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-yellow-600 border border-transparent rounded-md hover:bg-yellow-700">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                        </svg>
                        Edit
                    </a>
                    <a href="{{ route('admin-dashboard.location.cities.index') }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-600">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100">Airports ({{ $city->airports->count() }})</h3>
                            <a href="{{ route('admin-dashboard.location.airports.index') }}?city_id={{ $city->id }}" class="text-xs text-blue-600 hover:text-blue-800">View All</a>
                        </div>
                        @if($city->airports->count() > 0)
                            <div class="space-y-3">
                                @foreach($city->airports->take(10) as $airport)
                                    <div class="flex items-center justify-between py-2 border-b border-gray-200 dark:border-gray-600 last:border-0">
                                        <div>
                                            <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $airport->name }}</p>
                                            @if($airport->description)
                                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ Str::limit($airport->description, 60) }}</p>
                                            @endif
                                        </div>
                                        <div class="flex items-center space-x-2">
                                            <code class="text-xs bg-gray-200 dark:bg-gray-600 px-2 py-1 rounded">{{ $airport->code }}</code>
                                            @if($airport->is_active)
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100">Active</span>
                                            @else
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100">Inactive</span>
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
                                @if($city->airports->count() > 10)
                                    <p class="text-xs text-gray-500 pt-2">and {{ $city->airports->count() - 10 }} more airports...</p>
                                @endif
                            </div>
                        @else
                            <p class="text-sm text-gray-500">No airports added yet</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
                        ...
                        <div class="space-y-3">
                            <a href="{{ route('admin-dashboard.location.airports.create') }}?city_id={{ $city->id }}" class="block w-full text-center px-3 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md hover:bg-green-700">
                                Add Airport
                            </a>
                            <a href="{{ route('admin-dashboard.location.countries.show', $city->country->id) }}" class="block w-full text-center px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-600">
                                View Country
                            </a>
                        </div>
                    </div>
