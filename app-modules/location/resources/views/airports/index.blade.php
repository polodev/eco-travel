<x-admin-dashboard-layout::layout>
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
        ...
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Airport Management</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Manage airports for flight destinations</p>
                </div>
                <div class="flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('admin-dashboard.location.airports.create') }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md hover:bg-blue-700">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="overflow-hidden">
            <table id="airports-table" class="w-full divide-y divide-gray-200 dark:divide-gray-700">
    ...
    @push('scripts')
    <script>
        const current_route_name = 'admin-dashboard.location.airports.index';
        
        $(document).ready(function() {
            // DataTable configuration
            var airportsTable = $('#airports-table').DataTable({
                processing: true,
                serverSide: true,
                searchDelay: 500,
                pageLength: 25,
                ...
                responsive: false,
                ajax: {
                    url: '{{ route('admin-dashboard.location.airports.json') }}',
                    type: "POST",
                    data: function(d) {
                ...
                drawCallback: function() {
                    ...
                    $('#airports-table').addClass('divide-y divide-gray-200 dark:divide-gray-700');
                    $('#airports-table thead').addClass('bg-gray-50 dark:bg-gray-700');
                    $('#airports-table thead th').addClass('px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider');
                    $('#airports-table tbody').addClass('bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700');
                    $('#airports-table tbody td').addClass('px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100');
                    
                    // Fix column alignment after draw
                    setTimeout(function() {
                        airportsTable.columns.adjust();
                    }, 100);
                }
            });
            
            // Filter change listeners
            var filterElements = ['#country_id', '#city_id', '#is_active'];
            ...
            airportsTable.on('draw', function() {
                updateDataTableInfo();
            });
