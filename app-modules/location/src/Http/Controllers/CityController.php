...
class CityController extends Controller
{
    ...
    {
        $model = City::with('country')->withCount('airports');
        ...
        return DataTables::eloquent($model)
            ...
            ->addColumn('stats', function (City $city) {
                ...
                $html .= '<div>Airports: ' . $city->airports_count . '</div>';
                if ($city->population) {
                    $html .= '<div>Pop: ' . number_format($city->population) . '</div>';
                }
                $html .= '</div>';
                return $html;
            })
            ->addColumn('action', function (City $city) {
                ...
                $actions .= '<a href="' . route('admin-dashboard.location.cities.show', $city->id) . '" class="inline-flex items-center px-2.5 py-1.5 text-xs font-medium rounded text-white bg-blue-600 hover:bg-blue-700 transition-colors" title="View">
                ...
                $actions .= '<a href="' . route('admin-dashboard.location.cities.edit', $city->id) . '" class="inline-flex items-center px-2.5 py-1.5 text-xs font-medium rounded text-white bg-yellow-600 hover:bg-yellow-700 transition-colors" title="Edit">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin-dashboard.location.cities.index')
    ...
    public function update(Request $request, City $city)
    {
        ...
        return redirect()->route('admin-dashboard.location.cities.show', $city->id)
