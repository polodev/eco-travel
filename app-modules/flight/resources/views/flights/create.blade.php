    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Create Flight</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Add a new flight route</p>
                </div>
                <a href="{{ route('admin-dashboard.flight.flights.index') }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-600">
        ...
        <div class="p-6">
            <form action="{{ route('admin-dashboard.flight.flights.store') }}" method="POST" class="space-y-6">
                @csrf
                        ...
                        <div>
                            ...
                            <select id="airline_id" 
                                ...
                                @foreach($airlines as $airline)
                                    <option value="{{ $airline->id }}" {{ old('airline_id', request('airline_id')) == $airline->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select id="flight_type" 
                                ...
                                @foreach(\Modules\Flight\Models\Flight::getAvailableFlightTypes() as $value => $label)
                                    <option value="{{ $value }}" {{ old('flight_type') == $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="aircraft_type" 
                                ...
                                @foreach(\Modules\Flight\Models\Flight::getAvailableAircraftTypes() as $value => $label)
                                    <option value="{{ $value }}" {{ old('aircraft_type') == $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @foreach(\Modules\Flight\Models\Flight::getAvailableOperatingDays() as $value => $label)
                            <label class="flex items-center">
                                <input type="checkbox" 
                                       name="operating_days[]" 
                                       value="{{ $value }}" 
                                       {{ is_array(old('operating_days')) && in_array($value, old('operating_days')) ? 'checked' : '' }}
                ...
                <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <a href="{{ route('admin-dashboard.flight.flights.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-600">
                        Cancel
                    </a>
