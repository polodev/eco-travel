<x-admin-dashboard-layout::layout>
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
        ...
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Flight Management</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Manage flights for travel destinations</p>
                </div>
            ...
            <div id="filter_area" class="bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-3" style="display: block;">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <div>
                        ...
                        <input class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" type="text" name="search_text" id="search_text" placeholder="Search by flight number, route...">
                    </div>
                     <div>
                        <label for="airline_filter" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Airline</label>
                        <select name="airline_filter" id="airline_filter" 
                    ...
                    <div>
                        <label for="departure_filter" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Departure</label>
                        <select name="departure_filter" id="departure_filter" 
                                 class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
                            <option value="">All Airports</option>
                            @foreach($airports as $airport)
                                <option value="{{ $airport->id }}">{{ $airport->name }} ({{ $airport->iata_code }}) - {{ $airport->city->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <div class="overflow-hidden">
            <table id="flights-table" class="w-full divide-y divide-gray-200 dark:divide-gray-700">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var flightsTable = $('#flights-table').DataTable({
                processing: true,
                serverSide: true,
                searchDelay: 500,
                pageLength: 25,
                ...
                ajax: {
                    ...
                    data: function(d) {
                        ...
                        d.airline_id = $('#airline_filter').val();
                        d.departure_airport_id = $('#departure_filter').val();
                    }
                },
                columns: [
                    {
                ...
                drawCallback: function() {
                    ...
                    $('#flights-table').addClass('divide-y divide-gray-200 dark:divide-gray-700');
                    $('#flights-table thead').addClass('bg-gray-50 dark:bg-gray-700');
                    $('#flights-table thead th').addClass('px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider');
                    $('#flights-table tbody').addClass('bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700');
                    $('#flights-table tbody td').addClass('px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100');
                    
                    // Fix column alignment after draw
                    setTimeout(function() {
                        flightsTable.columns.adjust();
                    }, 100);
                }
            });
            
            // Filter change listeners
            var filterElements = ['#is_active', '#airline_filter', '#departure_filter'];
