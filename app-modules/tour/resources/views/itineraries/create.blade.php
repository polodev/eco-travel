    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Create Tour Itinerary</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Add a new day-by-day itinerary</p>
                </div>
                <a href="{{ route('tour::admin.itineraries.index') }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-600">
        ...
        <div class="p-6">
            <form action="{{ route('tour::admin.itineraries.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="tour_id"
                                ...
                                @foreach($tours as $tour)
                                    <option value="{{ $tour->id }}" {{ (old('tour_id', $selectedTourId) == $tour->id) ? 'selected' : '' }}>
                                        {{ $tour->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="day_description"
                                      ...
                                      required>{{ old('day_description') }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <textarea id="optional_activities"
                                      ...
                                      rows="4"
                                      class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                      placeholder="List optional activities (one per line)">{{ old('optional_activities') }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea id="transportation"
                                      ...
                                      rows="2"
                                      class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                      placeholder="Transportation methods (one per line)">{{ old('transportation') }}</textarea>
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center">
                                    <input type="checkbox" id="breakfast" name="meals_included[]" value="breakfast" 
                                           {{ in_array('breakfast', old('meals_included', [])) ? 'checked' : '' }}
                                ...
                                <div class="flex items-center">
                                    <input type="checkbox" id="lunch" name="meals_included[]" value="lunch"
                                           {{ in_array('lunch', old('meals_included', [])) ? 'checked' : '' }}
                                           class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                ...
                                <div class="flex items-center">
                                    <input type="checkbox" id="dinner" name="meals_included[]" value="dinner"
                                           {{ in_array('dinner', old('meals_included', [])) ? 'checked' : '' }}
                                           class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="meal_options"
                                      ...
                                      rows="2"
                                      class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                      placeholder="Available meal options (one per line)">{{ old('meal_options') }}</textarea>
                            @error('meal_options')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <select id="accommodation_type"
                                ...
                                @foreach($accommodationTypes as $key => $label)
                                    <option value="{{ $key }}" {{ old('accommodation_type') == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="accommodation_rating"
                                ...
                                <option value="1" {{ old('accommodation_rating') == '1' ? 'selected' : '' }}>1 Star</option>
                                <option value="2" {{ old('accommodation_rating') == '2' ? 'selected' : '' }}>2 Stars</option>
                                <option value="3" {{ old('accommodation_rating') == '3' ? 'selected' : '' }}>3 Stars</option>
                                <option value="4" {{ old('accommodation_rating') == '4' ? 'selected' : '' }}>4 Stars</option>
                                <option value="5" {{ old('accommodation_rating') == '5' ? 'selected' : '' }}>5 Stars</option>
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="special_notes"
                                      ...
                                      rows="2"
                                      class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                      placeholder="Any special notes or instructions for this day">{{ old('special_notes') }}</textarea>
                            @error('special_notes')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <a href="{{ route('tour::admin.itineraries.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-600">
                        Cancel
                    </a>
