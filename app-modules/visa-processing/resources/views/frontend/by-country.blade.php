<x-customer-frontend-layout::layout>
    <x-slot name="title">{{ $countryName }} {{ __('messages.visas') }} - {{ __('messages.visa_processing') }}</x-slot>
    <x-slot name="metaDescription">{{ __('messages.browse_country_visas_meta', ['country' => $countryName]) }}</x-slot>
    ...
    <div class="container mx-auto px-4 py-8">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{ $countryName }} {{ __('messages.visas') }}</span>
                    </div>
                </li>
            </ol>
        </nav>
            ...
            <div class="flex items-center space-x-4 mb-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
                        {{ $countryName }} {{ __('messages.visas') }}
                    </h1>
                    <p class="text-gray-600 dark:text-gray-400">
                        {{ __('messages.browse_country_description', ['country' => $countryName]) }}
                    </p>
                </div>
            </div>
        </div>
        ...
        @else
            ...
            <div class="text-center py-12">
                ...
                <p class="text-gray-500 dark:text-gray-400 mb-6">
                    {{ __('messages.no_country_visas_description', ['country' => $countryName]) }}
                </p>
        ...
        <div class="mt-12 bg-gray-50 dark:bg-gray-800 rounded-lg p-6">
            <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-4">
                {{ __('messages.about_country_visas', ['country' => $countryName]) }}
            </h2>
            <div class="prose prose-gray dark:prose-invert max-w-none">
                <p>{{ __('messages.country_visa_general_info', ['country' => $countryName]) }}</p>
            </div>
        </div>
        ...
        <div class="mt-8">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                {{ __('messages.other_popular_countries') }}
            </h3>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                @php
                    $popularCountries = \Modules\VisaProcessing\Models\VisaProcessing::published()
                ...
                @foreach($popularCountries as $country)
                    <a href="{{ route('visa-processing::visa-processings.by-country', $country->country_slug) }}" 
                       class="block p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:border-blue-500 dark:hover:border-blue-400 transition duration-200">
                        <div class="flex items-center space-x-2">
                            <span class="text-lg">{{ $country->country_flag }}</span>
                            <span class="font-medium text-gray-900 dark:text-white">{{ $country->country_name }}</span>
                        </div>
                    </a>
                @endforeach
            </div>
